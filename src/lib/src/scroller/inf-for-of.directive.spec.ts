import { InfiniteForModule } from './../module';
import { InfForOf } from './inf-for-of.directive';
import { CommonModule } from '@angular/common';
import { Component, Directive } from '@angular/core';
import { ComponentFixture, TestBed, async } from '@angular/core/testing';
import { By, ɵgetDOM as getDOM } from '@angular/platform-browser';
import { addMatchers, toHaveText, customMatchers } from "../../testing/jasmine-matchers";
...
describe('infForOf', () => {
  ...
  it('should iterate over an array of objects', async(() => {
    const template = '<ul><li *infFor="let item of items">{{item["name"]}};</li></ul>';
  ...
  it('should gracefully handle nulls', async(() => {
    const template = '<ul><li *infFor="let item of null">{{item}};</li></ul>';
    fixture = createTestComponent(template);
  ...
  xit('should throw on non-iterable ref and suggest using an array', async(() => {
    fixture = createTestComponent();

    getComponent().items = <any>'whaaa';
     //this error msg has been removed
     expect(() => fixture.detectChanges())
      .toThrowError(
      /Cannot find a differ supporting object 'whaaa' of type 'string'. infFor only supports binding to Iterables such as Arrays/);
  }));
  ...
  it('should repeat over nested arrays', async(() => {
    const template = '<div *infFor="let item of items">' +
      '<div *infFor="let subitem of item">{{subitem}}-{{item.length}};</div>|' +
      '</div>';
    fixture = createTestComponent(template);
  ...
  it('should repeat over nested ngIf that are the last node in the infFor template', async(() => {
    const template = `<div *infFor="let item of items; let i=index">` +
      `<div>{{i}}|</div>` +
  ...
  xit('should allow of saving the collection', async(() => {
    const template =
      '<ul><li *infFor="let item of [1,2,3] as items; index as i">{{i}}/{{items.length}} - {{item}};</li></ul>';
    fixture = createTestComponent(template);
  ...
  it('should display indices correctly', async(() => {
    const template = '<span *infFor ="let item of items; let i=index">{{i.toString()}}</span>';
    fixture = createTestComponent(template);
  ...
  it('should display first item correctly', async(() => {
    const template =
      '<span *infFor="let item of items; let isFirst=first">{{isFirst.toString()}}</span>';
    fixture = createTestComponent(template);
  ...
  it('should display last item correctly', async(() => {
    const template =
      '<span *infFor="let item of items; let isLast=last">{{isLast.toString()}}</span>';
    fixture = createTestComponent(template);
  ...
  it('should display even items correctly', async(() => {
    const template =
      '<span *infFor="let item of items; let isEven=even">{{isEven.toString()}}</span>';
    fixture = createTestComponent(template);
  ...
  it('should display odd items correctly', async(() => {
    const template =
      '<span *infFor="let item of items; let isOdd=odd">{{isOdd.toString()}}</span>';
    fixture = createTestComponent(template);
  ...
  it('should allow to use a custom template', async(() => {
    const template =
      '<ng-container *infFor="let item of items; template: tpl"></ng-container>' +
  ...
  it('should use a default template if a custom one is null', async(() => {
    const template =
      `<ul><ng-container *infFor="let item of items; template: null; let i=index">{{i}}: {{item}};</ng-container></ul>`;
    fixture = createTestComponent(template);
    getComponent().items = ['a', 'b', 'c'];
    fixture.detectChanges();
    detectChangesAndExpectText('0: a;1: b;2: c;');
  ...
  it('should use a custom template when both default and a custom one are present', async(() => {
    const template =
      '<ng-container *infFor="let item of items; template: tpl">{{i}};</ng-container>' +
  ...
  describe('track by', () => {
    it('should console.warn if trackBy is not a function', async(() => {
      ...
      const template = `<p *infFor="let item of items; trackBy: value"></p>`;
      fixture = createTestComponent(template);
    ...
    it('should track by identity when trackBy is to `null` or `undefined`', async(() => {
      ...
      const template = `<p *infFor="let item of items; trackBy: value">{{ item }}</p>`;
      fixture = createTestComponent(template);
    ...
    it('should set the context to the component instance', async(() => {
      const template =
        `<p *infFor="let item of items; trackBy: trackByContext.bind(this)"></p>`;
      fixture = createTestComponent(template);
    ...
    it('should not replace tracked items', async(() => {
      const template =
        `<p *infFor="let item of items; trackBy: trackById; let i=index">{{items[i]}}</p>`;
      fixture = createTestComponent(template);
    ...
    it('should update implicit local variable on view', async(() => {
      const template =
        `<div *infFor="let item of items; trackBy: trackById">{{item['color']}}</div>`;
      fixture = createTestComponent(template);
    ...
    it('should handle added and removed items properly when tracking by index', async(() => {
      const template = `<div *infFor="let item of items; trackBy: trackByIndex">{{item}}</div>`;
      fixture = createTestComponent(template);
    ...
    it('should support injecting `infFor` and get an instance of `infForOf`', async(() => {
      const template = `<ng-template infFor [infForOf]='items' let-item test></ng-template>`;
      fixture = createTestComponent(template);
      const testDirective = fixture.debugElement.childNodes[0].injector.get(TestDirective);
      const infForOf = fixture.debugElement.childNodes[0].injector.get(InfForOf);
      expect(testDirective.infFor).toBe(infForOf);
    }));
  });
});
...
class TestDirective {
  constructor(public infFor: InfForOf<any>) { }
}

const TEMPLATE = '<div><span *infFor="let item of items">{{item.toString()}};</span></div>';
